...
namespace Mikoto.RegionOverride
{
    internal class LERegistryRedirector
    {
        ...
        internal byte[] GetBinaryData()
        {
            ...
            {
                var srcBytes = _registryReplacement[i].ToByteArray();
                Array.Copy(srcBytes, 0, entrys, i * Marshal.SizeOf(_registryReplacement[i]), srcBytes.Length);
            }
            result = result.Concat(entrys).Concat(_objectData.ToArray()).ToArray();
        ...
        {
            AddObjectData("REG_SZ", value, out offset, out ulong temp);
